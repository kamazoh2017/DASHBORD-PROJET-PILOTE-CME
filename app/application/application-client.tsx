...
import { Users, UserCheck, Target, Calendar, Loader2, Database, AlertCircle, Info } from 'lucide-react';
...
export default function ApplicationClient() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Vérifier si les données du jour sont disponibles (après 16h pour les données application)
      const selectedDateObj = new Date(selectedDate);
      const now = new Date();
      // Pour les données d'application, disponibles après 16h
      ...
      try {
        const dailyRes = await fetch(`/api/application?date=${selectedDate}`);
        const daily = await dailyRes.json();
        setDailyData(Array.isArray(daily) ? daily : []);

        const cumRes = await fetch(`/api/application?fromDate=${PROJECT_START_DATE.toISOString().split('T')[0]}&toDate=${selectedDate}`);
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          ...
          Reporting d'Application
        </h1>
        <p className="text-gray-600">Données extraites de la base de données Sage-Femme Infos (2ème lien Google Sheets - feuilles CSV)</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <div className="text-sm">
          <p className="font-medium text-blue-800">Source des données</p>
          <p className="text-blue-600">
            Les données proviennent des feuilles CSV du 2ème lien Google Sheets. 
            <br />• <strong>SF Formées</strong> : données du 1er lien (supervision)
            <br />• <strong>SF Présentes</strong> : comptées dans la feuille CSV du jour
            <br />• <strong>FE Reçues</strong> : données du 1er lien (supervision)
            <br />• <strong>FE Enregistrées = FE Connectées</strong> : même valeur (le lien ne distingue pas encore ces deux données)
          </p>
        </div>
      </div>
      ...
      {!dataAvailable && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-600">Les données d'application du jour sont disponibles à partir de 16h (ou J+1). Sélectionnez une date antérieure.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 mb-6">
        ...
        <DataTable 
          data={dailyData} 
          title={`📅 Tableau 1 : Données du ${formatDateFR(selectedDate)} (Base de données)`} 
          showTotal 
          hideDate
        />
        ...
        <CumulativeTableApp 
          data={cumulativeData}
          title={`📊 Tableau 2 : Données cumulées du 03/02/2026 au ${formatDateFR(selectedDate)} (Base de données)`}
        />
      </div>
